...
| `Esc`                                                            | Cancel a pen path, end a road line, or return to the Select tool |
...
| Region                                     | Holds                                                                                                                     |
| ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------- |
| Tool rail, down the left                   | The four tools, then the shape buttons, then Text                                                                         |
...
| Right panel                                | Everything else that acts on what is selected: SELECTION, ELEMENT, ANIMATION, POINTS while the edit points tool is active, and ROADS while the roads tool is active |
...
Every button with a key has the key, with its name and description, printed in the button's hover label.

> [!TIP]
> Hover the mouse over each button if you forget a key.
...
| Action                  | Description                                                                                                                                                                                  |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| New line                | Starts a new road line. Each click on the canvas adds a corner. The corner moves to the nearest direction the line can take: across, up and down, or 45 degrees. A dashed line shows where the next click puts the corner. |
| `Enter` or `Esc`        | Ends the line you are drawing.                                                                                                                                                               |
| `Delete` or `Backspace` | Removes the last corner of the line you are drawing.                                                                                                                                         |
| Extend                  | Continues the selected line from its last corner.                                                                                                                                            |
| Click a line            | Selects the line. Its corners turn pink.                                                                                                                                                     |
| Drag a corner           | Moves the corner. The two parts of the line beside it still run across, up and down, or at 45 degrees.                                                                                       |
| Click a stop            | Selects the stop.                                                                                                                                                                            |
| Line name               | Names the selected line. The Lines box lists the line by this name. A line with no name is a branch of another line, and the Lines box does not list it.                                    |
| Line colour             | Picks the color of the selected line from a list of 20 colors. A new line gets the first color that no other line uses.                                                                     |
| Station                 | The next click puts a station on the nearest line. A station is a white circle.                                                                                                              |
| Depot                   | The next click puts a depot on the nearest line. A depot is a small box with a label. It hangs below the line.                                                                               |
| Connect                 | Moves the last corner of the selected line onto the nearest station of another line. Both lines then stop at that station. A station where two or more lines stop is drawn as a double circle. |
| Disconnect              | Pulls the last corner of the selected line back off the shared station. The other line keeps its station.                                                                                    |
| Stop label              | The text beside the selected stop.                                                                                                                                                           |
| Stop identifier         | A second text for the selected stop. The map does not draw it. It shows when a reader holds the mouse over the label, and a screen reader reads it aloud.                                     |
| Label side              | Moves the label of the selected stop to the next of eight places around the stop.                                                                                                            |
| Delete                  | Removes the selected stop. With no stop selected, it removes the whole selected line.                                                                                                       |
| Map title               | The title of the map. Typing a title shows the title box in the top left corner.                                                                                                             |
| Title                   | Shows or hides the title box.                                                                                                                                                                |
| Key                     | Shows or hides the Lines box and the Symbols box. The Lines box shows each named line with its color. The Symbols box explains the double circle, the circle and the box.                   |

The Symbols box always uses the same three sentences: "a place two or more
lines meet", "a step on one line", and "a button, a key or a file the step
uses".

The map belongs to the capsule it is drawn on, and it is drawn behind the
shapes of that capsule. Undo and Redo work on the map. Save project keeps
the map, and Load brings it back. Download puts the map in the SVG file of
the capsule. When Export size is set to shapes, the file grows to fit the
whole map. Import reads a map file back as ordinary shapes, not as a map
the roads tool can edit.
